...
class e4e(nn.Module):
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            ...
            encoder_ckpt = self.__get_keys(ckpt, 'encoder')

            # if self.opts.input_nc != 6:
            #     print ("here ...")
            #     shape = encoder_ckpt['input_layer.0.weight'].shape
            #     altered_input_layer1 =  0.*torch.randn(shape[0], self.opts.input_nc, shape[2], shape[3], dtype=torch.float32)
            #     altered_input_layer1[:, :6, :, :] = encoder_ckpt['input_layer.0.weight']

            #     altered_input_layer2 =  0.*torch.randn(shape[0], self.opts.input_nc, shape[2], shape[3], dtype=torch.float32)
            #     altered_input_layer2[:, 6:, :, :] = encoder_ckpt['input_layer.0.weight']
            #     encoder_ckpt['input_layer_updated_canvas.0.weight'] = altered_input_layer2
            #     encoder_ckpt['input_layer.0.weight'] = altered_input_layer1
    ...
    def forward(self, x, target_id_feat=None, latent=None, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
                inject_latent=None, return_latents=False, alpha=None, average_code=False, input_is_full=False, get_multiple_codes=False):
        
        if input_code:
            codes = x
        else:
            if get_multiple_codes:
                codes_canvas0, codes_canvas1 = self.encoder(x,target_id_feat,get_multiple_codes)
                codes = torch.cat([codes_canvas0, codes_canvas1],axis=0)    
            else:
                codes = self.encoder(x,target_id_feat)
                # print (codes.shape,self.latent_avg.shape,self.latent_avg.repeat(codes.shape[0], 1, 1).shape)
            # residual step
            if x.shape[1] == self.opts.input_nc and latent is not None:
